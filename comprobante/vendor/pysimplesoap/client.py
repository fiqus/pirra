...
from comprobante.vendor.pysimplesoap.simplexml import SimpleXMLElement, REVERSE_TYPE_MAP, OrderedDict
...
<%(soap_ns)s:Body>
    ...
    def wsdl_parse(self, url, debug=False, cache=False):
        ...
        if cache:
            ...
            filename_pkl = "%s.pkl" % hashlib.md5(url.encode()).hexdigest()
        ...
        def fetch(url):
            ...
            # make md5 hash of the url for caching... 
            filename = "%s.xml" % hashlib.md5(url.encode()).hexdigest()
            if isinstance(cache, str):
