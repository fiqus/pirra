...
class OrderedDict(dict):
    ...
    def __new__(cls, *args, **kwargs):
        n = super(OrderedDict, cls).__new__(cls, *args, **kwargs)
        n.dict_keys = []
        return n
    ...
    def insert(self, key, value, index=0):
        if key not in self.dict_keys:
            self.dict_keys.insert(index, key)
        super(OrderedDict, self).__setitem__(key, value)
    ...
    def __iter__(self):
        return iter(self.dict_keys)

    def keys(self):
        return self.dict_keys

    def items(self):
        return [(key, self[key]) for key in self.dict_keys]
    ...
    def __str__(self):
        return "*%s*" % super(OrderedDict, self).__str__()
