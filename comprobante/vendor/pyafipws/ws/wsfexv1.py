...
import os
import sys
from comprobante.vendor.pyafipws.utils import inicializar_y_capturar_excepciones, BaseWS, get_install_dir
        ...
        if res:
            for pu in res:
                ...
                try:
                    ...
                    print(e)
                ret.append(r)
        return [("%(nro)s\tBloqueado:%(bloqueado)s\tFchBaja:%(baja)s" % r).replace("\t", sep) for r in ret]
